...
for (var i = 0; i < maxNumParticles; i++)
  d2[i] = new Float32Array(maxNumParticles);
...
window.onload = function init() {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    16 * (maxNumParticles + numVertices),
    gl.STATIC_DRAW
  );
  ...
  if (repulsion)
    for (var ParticleK = 0; ParticleK < numParticles; ParticleK++) {
      ...
      if (ParticleK != ParticleI) {
        var t = normalize(
          subtract(
            particleSystem[ParticleI].position,
...
var collision = function (particleId) {
  ...
  for (var i = 0; i < 3; i++) {
    if (particleSystem[particleId].position[i] >= 1.0) {
      particleSystem[particleId].velocity[i] =
        -coef * particleSystem[particleId].velocity[i];
